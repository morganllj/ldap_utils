#!/usr/bin/perl -w
#
# morgan@morganjones.org give an arbitrary search walk the values of
# the password hash and update them.  the use case was converting
# \{crypt\}\*(NP|LK|disabled)\*/ to \{SHA\}\*(NP|LK|disabled)\*/ for a
# move to google apps
...
my $ldap = Net::LDAP->new("ldap0.domain.org") || die "$@";
...
for my $entry ($rslt->entries) {
    ...
    if (defined $pass && $pass =~ /\{crypt\}\*(NP|LK|disabled)\*/) {
	print "$dn /$pass ";
	$pass =~ s/\{crypt\}/\{SHA\}/;
	print "replace with /$pass/\n";
